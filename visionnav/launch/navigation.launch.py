...
def generate_launch_description():
    params = os.path.join(get_package_share_directory('visionnav'), 'config', 'nav2_params.yaml')
    return LaunchDescription([
        ...
        Node(package='nav2_lifecycle_manager', executable='lifecycle_manager', name='lifecycle_manager_navigation',
             ...
             parameters=[{'autostart': True, 'node_names': ['planner_server', 'smoother_server'],
                          'bond_timeout': 0.0}]),
        Node(package='visionnav', executable='semantic_costmap', name='semantic_costmap', output='screen'),
        Node(package='visionnav', executable='semantic_navigator', name='semantic_navigator', output='screen'),
    ])
