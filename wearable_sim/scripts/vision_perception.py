...
====================
ROS 2 Jazzy – Wearable Blind-Assist Vision Node  (Ultralytics YOLOv8-seg edition)
...
SMALL_OBJECTS = {"cup", "bottle", "cell phone", "mouse", "remote", "book", "vase", "clock",
...
FLOOR_OBJECTS = {"chair", "couch", "bed", "dining table", "toilet", "refrigerator", "oven",
...
DEPTH_WEIGHTS = os.path.join(_SCRIPT_DIR, "depth_anything_v2_metric_indoor_vits.pth")
...
MODE_PARAMS = {
    "indoor": {
        "conf_threshold":       0.40,    # track confirmation (MIN_HITS) now filters hallucinations
...
class VisionPerceptionNode(Node):
    ...
    def _load_model(self):
        ...
        # Newer Ultralytics replaced half=True with quantize="fp16" (and warns every frame otherwise)
        if "quantize" in DEFAULT_CFG_DICT:
            self._precision_kwargs = {"quantize": "fp16"} if self._use_half else {}
        else:
            self._precision_kwargs = {"half": self._use_half}
    ...
    def _in_view(self, x: float, y: float, z_mid: float, pose, K, h: int):
        """Should the camera be seeing a mapped object at (x, y, z_mid) right now?

        Returns (in_view, u, v, cam_depth): the pixel it would project to and its distance along the
        camera axis, so a caller can check whether something closer occludes it (dmap[v, u] < cam_depth).
        """
        fx, fy, cx, cy, w = K
        px, py, yaw = pose
        dx, dy = x - px, y - py
        c, s = math.cos(yaw), math.sin(yaw)
        p_base = np.array([c * dx + s * dy, -s * dx + c * dy, z_mid])
        if math.hypot(p_base[0] - self._cam_t[0], p_base[1] - self._cam_t[1]) > VISIBILITY_MAX_RANGE:
            return False, 0, 0, 0.0
        pc = self._cam_R.T @ (p_base - self._cam_t)
        if pc[2] < 0.5:
            return False, 0, 0, 0.0
        u = fx * pc[0] / pc[2] + cx
        v = fy * pc[1] / pc[2] + cy
        in_view = 0.12 * w < u < 0.88 * w and 0.12 * h < v < 0.88 * h
        return in_view, u, v, float(pc[2])

    def _occluded(self, u: float, v: float, cam_depth: float, dmap, w: int, h: int) -> bool:
        """True if the metric depth map shows something clearly closer at this pixel."""
        if dmap is None:
            return False
        ui, vi = int(u), int(v)
        ...
        for label, tracks in self._static_tracks.items():
            ...
            for t in tracks:
                if id(t) not in matched and not near_user(t):
                    in_view, u, v, cam_depth = self._in_view(t.x[0], t.x[1], t.z + 0.5 * t.height, pose, K, h)
                    if in_view and not self._occluded(u, v, cam_depth, dmap, w, h):
